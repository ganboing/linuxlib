...
#if 0
static void shadow_pages(void* ptr, size_t len, int prot) {
	auto base = (char*)ptr - (uintptr_t(ptr) % pagesz);
	len += (char*)ptr - base;
	len = (len % pagesz) ? pagesz * (len / pagesz + 1): len;
	auto file = cppfmt("/dev/shm/vdso-shadow-%ld-%p-%zu", (long)getpid(), base, len);
	int fd = open(file.c_str(), O_CREAT | O_EXCL | O_RDWR, 0);
	if (fd < 0){
		FAIL ("failed to create shadow file");
	}
	int ret = unlink(file.c_str());
	if (ret < 0){
		FAIL("unlink failed");
	}
	ssize_t s = write(fd, base, len);
	if (s < 0 || (size_t)s != len) {
		FAIL("write failed");
	}
	void* m = mmap(base, len, prot, MAP_FIXED | MAP_PRIVATE | MAP_FILE, fd, 0);
	if (m == MAP_FAILED) {
		FAIL("mmap failed");
	}
	close(fd);
...
int my___vdso_gettimeofday(struct timeval *tv, void *tz) noexcept {
